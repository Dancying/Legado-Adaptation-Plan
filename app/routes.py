...
def legado_search():
    logger.info(f"[{request.remote_addr}] >>> Request Search >> Params: {request.args.to_dict()}")
...
def legado_proxy():
    logger.info(f"[{request.remote_addr}] >>> Request Proxy >> Params: {request.args.to_dict()}")
    url = request.values.get("url")
    if not url:
...
def handle_404(e):
    logger.warning(f"[{request.remote_addr}] >>> 404 Not Found >> {request.url} > Params: {request.values.to_dict()}")
...
def legado_logs():
    logger.debug(f"[{request.remote_addr}] >>> Request Logs >> Params: {request.args.to_dict()}")
